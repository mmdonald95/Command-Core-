...
export default function VisualEditAgent() {
	...
	const handleMouseOver = (e) => {
		...
		// Prevent hover effects on SVG elements (path, rect, circle, etc.)
		const tagName = e.target.tagName.toLowerCase();
		if (['path', 'rect', 'circle', 'line', 'polygon', 'polyline', 'ellipse', 'g'].includes(tagName)) {
			clearHoverOverlays();
			return;
		}

		// Prefer data-source-location, fallback to data-visual-selector-id  
		const selectorId = e.target.dataset.sourceLocation || e.target.dataset.visualSelectorId;
		const useSourceLocation = !!e.target.dataset.sourceLocation;
	...
	const handleElementClick = (e) => {
		...
		// Prevent clicking on SVG elements (path, rect, circle, etc.)
		const tagName = e.target.tagName.toLowerCase();
		if (['path', 'rect', 'circle', 'line', 'polygon', 'polyline', 'ellipse', 'g'].includes(tagName)) {
			return;
		}
		...
		// Prefer data-source-location, fallback to data-visual-selector-id
		const visualSelectorId = e.target.dataset.sourceLocation || e.target.dataset.visualSelectorId;
		const useSourceLocation = !!e.target.dataset.sourceLocation;
		...
		const elementData = {
			...
			classes: e.target.className?.baseVal || e.target.className || '',
			visualSelectorId: visualSelectorId,
			content: e.target.innerText,
			dataSourceLocation: e.target.dataset.sourceLocation,
			isDynamicContent: e.target.dataset.dynamicContent === 'true',
			linenumber: e.target.dataset.linenumber, // Keep for backward compatibility
			filename: e.target.dataset.filename, // Keep for backward compatibility
	...
	const updateElementClasses = (visualSelectorId, classes, replace = false) => {
		...
		elements.forEach(element => {
			// Check if element is SVG (has baseVal property on className)
			...
			if (isSVG) {
				...
				// For HTML elements, use className property
				if (replace) {
	...
	// Only run in iframe, not parent frame
